...
function generateLogicChallenge() {
    ...
    // Display pattern with new classes
    ...
    // Add question mark with special styling
...
function generateMoneyCounting() {
    ...
    const options = [correctAnswer, ...wrongAnswers.slice(0, 2)]
        .sort(() => Math.random() - 0.5)
        .slice(0, 3);
...
function checkMoney(isCorrect) {
    if (isCorrect) {
        ...
        buddyReact('happy');
        earnStar();
        document.getElementById('moneyScore').textContent = '⭐ Score: ' + (parseInt(document.getElementById('moneyScore').textContent.split(': ')[1]) + 1);
...
function generateRhymeChallenge() {
    ...
    document.getElementById('rhymeWord').textContent = currentRhyme.word.toUpperCase();
    document.getElementById('targetRhymeWord').textContent = currentRhyme.word.toLowerCase();
    
    // Generate options with new styling
    ...
    currentRhyme.rhymes.forEach(word => {
        ...
        btn.innerHTML = `
            <div style="font-size: 2.5rem; font-weight: 700; color: #2D3748;">${word}</div>
        `;
    ...
    */
    ...
    backButtons.forEach(btnId => {
        document.getElementById(btnId)?.addEventListener('click', () => {
            ...
            if (buddySpeech) {
                buddySpeech.querySelector('.speech-text').textContent = 'Welcome back! 🏡';
            }
        });
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    buddyCenter.addEventListener('click', () => {
        ...
        buddySpeech.querySelector('.speech-text').textContent = buddyMessages[messageIndex];
    ...
    activityBubbles.forEach(bubble => {
        bubble.addEventListener('mouseenter', function() {
            ...
            buddySpeech.querySelector('.speech-text').textContent = message;
        });
