...
// Get Supabase client (initialized in supabase-config.js)
let supabase = null;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (window.supabaseClient) {
        ...
        // Show error to user
        showAuthError('Could not connect to database. Please refresh.');
    }
});
...
function updateUserDisplay() {
    if (currentProfile && currentProfile.child_name) {
        const displayElement = document.getElementById('childNameDisplay');
        if (displayElement) {
            displayElement.textContent = `Hi, ${currentProfile.child_name}! 👋`;
        }
        
        // Update speech bubble
        if (currentProfile.child_name) {
            setTimeout(() => {
                updateSpeechBubble(`Hi ${currentProfile.child_name}! Ready to learn? 🎉`);
            }, 1500);
        }
    }
}
...
function generatePhonicsChallenge() {
    ...
    currentPhonicsWord.sounds.forEach(sound => {
        const soundDiv = document.createElement('div');
        soundDiv.className = 'sound-part';
    ...
    allOptions.forEach(option => {
        ...
        btn.innerHTML = `
            <span class="phonics-option-emoji">${option.emoji}</span>
            <span class="phonics-option-text">${option.word}</span>
        `;
        btn.addEventListener('click', () => checkPhonics(option.correct));
        optionsContainer.appendChild(btn);
    });
}

async function checkPhonics(isCorrect) {
    if (isCorrect) {
        playSound('correct');
        AppState.phonicsScore++;
        await saveProgress('phonics', AppState.phonicsScore, 1);
...
async function checkPattern(selectedOption) {
    if (selectedOption === currentPattern.answer) {
        playSound('correct');
        AppState.logicScore++;
        await saveProgress('logic', AppState.logicScore, 1);
...
function updateCoinDisplay() {
    ...
    coinDisplay.textContent = coinData.emoji;
    coinDisplay.style.background = coinData.color;
    
    document.getElementById('coinValue').textContent = coinData.display;
    document.getElementById('coinName').textContent = coinData.name;
    
    // Trigger animation
    document.getElementById('coinCard').style.animation = 'none';
    setTimeout(() => {
        document.getElementById('coinCard').style.animation = '';
    }, 10);
...
function generateMoneyCounting() {
    const questionContainer = document.getElementById('moneyQuestion');
    ...
    for (let i = 0; i < numCoins; i++) {
        ...
        coinDiv.className = 'coin-to-count';
        coinDiv.textContent = coin.emoji;
        coinDiv.style.background = coin.color;
    ...
    options.forEach(value => {
        const btn = document.createElement('button');
        btn.className = 'money-option';
        btn.textContent = value + '¢';
        btn.addEventListener('click', () => checkMoney(value === correctAnswer));
        optionsContainer.appendChild(btn);
    });
}

function checkMoney(isCorrect) {
    if (isCorrect) {
        playSound('correct');
...
function generateTimeChallenge() {
    ...
    options.forEach(time => {
        ...
        btn.addEventListener('click', () => checkTime(time === correctTime));
        optionsContainer.appendChild(btn);
    });
}
...
function updateClockDisplay() {
    ...
    digitalTime.textContent = `${currentTime.hour}:${currentTime.minute.toString().padStart(2, '0')}`;
}
...
async function checkTime(isCorrect) {
    if (isCorrect) {
        playSound('correct');
        AppState.timeScore++;
        await saveProgress('time', AppState.timeScore, 1);
...
function updateLetterDisplay() {
    ...
    letterData.words.forEach(word => {
        ...
        exampleDiv.innerHTML = `
            <span class="example-emoji">${word.emoji}</span>
            <span class="example-text">${word.word}</span>
        `;
        examplesContainer.appendChild(exampleDiv);
    });
    
    document.getElementById('letterCard').style.animation = 'none';
    setTimeout(() => {
        document.getElementById('letterCard').style.animation = '';
    }, 10);
}
...
function updateNumberDisplay() {
    ...
    document.getElementById('numberCard').style.animation = 'none';
    setTimeout(() => {
        document.getElementById('numberCard').style.animation = '';
    }, 10);
}
...
function generateShapeChallenge() {
    ...
    shuffled.forEach(shape => {
        ...
        btn.addEventListener('click', () => checkShape(shape.name));
        optionsContainer.appendChild(btn);
    });
}
...
async function checkShape(selectedShape) {
    if (selectedShape === currentShape.name) {
        playSound('correct');
        AppState.shapeScore++;
        await saveProgress('shapes', AppState.shapeScore, 1);
...
function updateColorDisplay() {
    ...
    document.getElementById('colorSwatch').style.backgroundColor = colorData.hex;
    ...
    colorData.examples.forEach(example => {
        const exampleDiv = document.createElement('div');
        exampleDiv.className = 'color-example';
        exampleDiv.innerHTML = `
            <span class="color-example-emoji">${example.emoji}</span>
            <span class="color-example-text">${example.text}</span>
        `;
        examplesContainer.appendChild(exampleDiv);
    });
    
    document.getElementById('colorCard').style.animation = 'none';
    setTimeout(() => {
        document.getElementById('colorCard').style.animation = '';
    }, 10);
}
...
function generateRhymeChallenge() {
    ...
    document.getElementById('rhymeEmoji').textContent = currentRhyme.emoji;
    document.getElementById('rhymeWord').textContent = currentRhyme.word;
    
    const optionsContainer = document.getElementById('rhymeOptions');
    optionsContainer.innerHTML = '';
    
    currentRhyme.rhymes.forEach(word => {
        const btn = document.createElement('button');
        btn.className = 'rhyme-option';
        btn.textContent = word;
        btn.addEventListener('click', () => checkRhyme(word));
        optionsContainer.appendChild(btn);
    });
}

async function checkRhyme(selectedWord) {
    if (currentRhyme.correct.includes(selectedWord)) {
        playSound('correct');
        AppState.rhymeScore++;
        await saveProgress('rhymes', AppState.rhymeScore, 1);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    backButtons.forEach(btnId => {
        document.getElementById(btnId)?.addEventListener('click', () => {
            if (btnId === 'backToWelcome') {
                switchScreen('welcomeScreen');
                updateSpeechBubble('What do you want to do? 😊');
            } else {
                switchScreen('activityScreen');
            }
        });
    ...
    document.getElementById('prevCoin')?.addEventListener('click', () => {
        playSound('click');
        AppState.currentCoin = (AppState.currentCoin - 1 + Coins.length) % Coins.length;
        updateCoinDisplay();
    });
    
    document.getElementById('nextCoin')?.addEventListener('click', () => {
        playSound('click');
        AppState.currentCoin = (AppState.currentCoin + 1) % Coins.length;
        updateCoinDisplay();
    });
...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR5YmVpdWtmc3V6d2R2andhdnRwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk1MjE0ODUsImV4cCI6MjA4NTA5NzQ4NX0.1DI8_CQpxNdtxhhq9-dAxObFxxD-5xan0MvUzNuxd-Y';
...
async function saveProgress(activity, score) {
    // Uncomment and configure when ready
    /*
    try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/progress`, {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('signupBtn')?.addEventListener('click', async () => {
        ...
        if (age < 3 || age > 10) {
            showAuthError('Age must be between 3 and 10');
            return;
        }
        ...
        try {
            ...
            // Save user profile
            await saveUserProfile(data.user.id, name, age, email);
            ...
            // Success!
            hideLoginScreen();
            updateSpeechBubble(`Welcome, ${name}! Let's learn together! 🎉`);
            playSound('celebrate');
...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('loginBtn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            hideLoginScreen();
            updateSpeechBubble(`Welcome back, ${profile?.full_name || 'friend'}! 🎉`);
            playSound('celebrate');
...
async function loadUserProfile(userId) {
    ...
    if (profile) {
        // Could display user name somewhere
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/progress`, {
            ...
            body: JSON.stringify({
                ...
                activity: activity,
                score: score,
                stars_earned: AppState.starsEarned,
                completed: score > 0,
